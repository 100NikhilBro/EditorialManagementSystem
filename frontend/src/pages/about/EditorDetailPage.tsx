...
const EditorDetailPage: React.FC = () => {
  ...
  const isOwnProfile = currentUser?._id === id;
  const canEdit =
    isOwnProfile && (editor?.roles?.editor || editor?.roles?.editorInChief);
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-10">
      <div className="max-w-4xl mx-auto bg-white border border-gray-300">
        ...
        {/* HEADER */}
        <div className="bg-gray-100 border-b border-gray-300 px-8 py-5 flex justify-between items-center">
          <button
            onClick={() => navigate('/editorial-board')}
          ...
          {canEdit && !isEditing && (
            <button
              onClick={() => setIsEditing(true)}
              className="text-sm border px-3 py-1 bg-blue-800 text-white"
            >
              Edit Profile
            </button>
          )}
        ...
        <div className="px-8 py-8 space-y-8 text-sm">
          ...
          {/* IMAGE */}
          {(editor.profile?.image || editData.image) && (
            <div>
              <img
                src={isEditing ? editData.image : editor.profile?.image}
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p>{editor.profile?.title || 'N/A'}</p>
                )}
              </div>

              {/* DEPARTMENT */}
              <div>
                <label className="text-gray-600 text-xs">Department</label>
                {isEditing ? (
                  <input
                    value={editData.department || ''}
                    onChange={(e) =>
                  ...
                  <p>{editor.profile?.department || 'N/A'}</p>
                )}
              </div>

              {/* AFFILIATION */}
              <div className="md:col-span-2">
                <label className="text-gray-600 text-xs flex gap-1">
                  <Building size={12} /> Affiliation
                </label>
                {isEditing ? (
                  <input
                    value={editData.affiliation || ''}
                    onChange={(e) =>
                      setEditData({ ...editData, affiliation: e.target.value })
                    }
                    className="w-full border px-2 py-1 mt-1"
                  />
                ) : (
                  <p>{editor.profile?.affiliation || 'N/A'}</p>
                )}
              </div>
          ...
          <section className="border-t pt-6">
            ...
            {isEditing ? (
              ...
              <p className="text-gray-600 whitespace-pre-wrap">
                {editor.profile?.bio || 'No biography provided'}
              </p>
            )}
          </section>
          ...
          <section className="border-t pt-6">
            ...
            {isEditing ? (
              ...
              <div className="flex flex-wrap gap-2">
                {editor.profile?.expertise?.map((e: string, i: number) => (
